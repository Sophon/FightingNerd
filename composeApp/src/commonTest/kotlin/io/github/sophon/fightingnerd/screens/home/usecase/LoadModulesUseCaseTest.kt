...
class LoadModulesUseCaseTest {
    ...
    fun `given features with no preferences when invoke then emits all features enabled by default`() = runTest {
        ...
        val features = listOf(
            FakeModule(featureInfo1),
            FakeModule(featureInfo2)
        )
        val mockRegistry = createRegistry(features)
        val mockStore = FakeDataStore()
        val useCase = LoadEmptyWidgetsUseCase(mockRegistry, mockStore)
    ...
    // region Test Helpers
    private suspend fun createRegistry(features: List<Module>): FeatureRegistry {
    ...
    private class FakeFeatureListLoader(
        ...
        override suspend fun loadFeatureList(): Config {
            val features = enabledFeatureNames.map { name ->
                Config.Feature(name = name, isEnabled = true, listOf())
            }
