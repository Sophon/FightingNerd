...
object BBFilters {
    object Invincible: Filter {
        ...
        override val predicate: (Move) -> Boolean = { move ->
            ...
            val costsMeter = (move.gameProperties as? BBMoveProperties)?.type.orEmpty().contains("super", ignoreCase = true)
                    || move.input.endsWith("od", ignoreCase = true)
                    || (move.gameProperties as? BBMoveProperties)?.type.orEmpty().contains("astral", ignoreCase = true)
